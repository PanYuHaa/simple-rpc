package org.peterpan.rpc.protocol.serialization.impl;
...
public class HessianSerialization implements IRpcSerialization {
   ...
   public <T> byte[] serialize(T object) {
      ...
      try (ByteArrayOutputStream os = new ByteArrayOutputStream()) {
         ...
         throw new SerializationException("Serialization error", e); // 抛出序列化异常，并包装原始异常信息
      }
   ...
   public <T> T deserialize(byte[] bytes, Class<T> clz) {
      ...
      try (ByteArrayInputStream is = new ByteArrayInputStream(bytes)) {
         ...
         throw new SerializationException("Deserialization error", e); // 抛出反序列化异常，并包装原始异常信息
      }
